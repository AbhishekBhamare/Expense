...
export default function Sidebar() {
  ...
  const [spending, setSpending] = useState(
    window.location.pathname === '/spending' ? true : false
  );
  const [categories, setCategories] = useState(
    window.location.pathname === '/categories' ? true : false
  );
  const [budget, setBudget] = useState(
    window.location.pathname === '/budget' ? true : false
  );
  const [user, setUser] = useState(
    window.location.pathname === '/user' ? true : false
  );
  ...
  const getUserData = useCallback(() => {
    if (id) {
      axios.get('https://expense-api-5ehm.onrender.com/user', {
        params: { id: id }
      }).then((response) => {
  ...
  const handlelogout = () => {
    ...
    setTimeout(() => deleteCookie('token'), 1000);
    setTimeout(() => setUserData({ key: null }), 1000);
    ...
    <>
      ...
      <div className={`fixed animate-fadeinleft w-72 z-50  bg-black h-dvh max-lg:${!isHamburgerPressed ? 'hidden' : 'visible'} `}>
        ...
        <div className='px-10 text-lg font-normal text-white text-start'>
          ...
          <div className={`flex py-3 my-1 px-2 ${dashboard?'':'hover:bg-secondary'} rounded-[20px]
         ...
         `}>
          ...
          <div className={`flex py-3 my-1 px-2 ${spending?'':'hover:bg-secondary'} rounded-[20px]
            ${spending?'bg-secondary ':''}
         `}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
          ...
          <div className={`flex py-3 my-1 px-2 ${budget?'':'hover:bg-secondary'} rounded-[20px]
            ${budget?'bg-secondary ':''}
         `}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
          ...
          <div className={`flex py-3 my-1 px-2 ${categories?'':'hover:bg-secondary'} rounded-[20px]
            ${categories?'bg-secondary ':''}
         `}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
          ...
          <div className={`flex py-3 my-1 px-2 ${user?'':'hover:bg-secondary'} rounded-[20px]
            ${user?'bg-secondary ':''}
         `}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
          ...
          <div className={`flex py-3 my-1 px-2 hover:bg-secondary rounded-[20px]
          `}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
      ...
      <div className='fixed bg-primary h-20 w-screen top-0 left-72 -z-20  max-lg:left-0'>
        <div className='flex items-center p-3 text-white '>
          <div className='invisible text-black max-lg:visible max-lg:flex max-lg:items-center'>
            <button
            onClick={() => setHamburgerPressed(!isHamburgerPressed)}>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-7">
                <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 5.25h16.5m-16.5 4.5h16.5m-16.5 4.5h16.5m-16.5 4.5h16.5" />
              </svg>
            </button>
          </div>
          <h1 className='text-xl font-semibold text-ternary flex justify-center items-center mx-3 max-sm:text-base'>Welcome, &nbsp;
