...
export default function Spending() {
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (cost && category && date) {
      await axios.post('https://expense-api-evxi.onrender.com/spending', {
        id: userData.key.id,
        cost: cost,
        category: category,
        date: date,
  ...
  const fetchData = useCallback(async () => {
    if (userData && userData.key && userData.key.id) {
      await axios.get('https://expense-api-evxi.onrender.com/spending', {
  ...
  const handleEdit = async() => {
    ...
    await axios.patch('https://expense-api-evxi.onrender.com/spending', entry)
      .then((res) => {
        setEditModal(false);
        fetchData();
      })
  ...
  const handleDelete = async (id) => {
    return axios.delete('https://expense-api-evxi.onrender.com/spending', { data: { id } })
      .then((res) => {
        fetchData();
