...
export default function Budget() {
  ...
  const fetchBudgetData = useCallback(() => {
    if (userData && userData.key && userData.key.id) {
      axios.get('https://expense-api-5ehm.onrender.com/budgets', {
  ...
  const fetchCategories = useCallback(() => {
    if (userData && userData.key && userData.key.id) {
      axios.get('https://expense-api-5ehm.onrender.com/categories', {
        params: { id: userData.key.id },
      }).then((response) => {
  ...
  const fetchIncome = useCallback(() => {
    if (userData && userData.key && userData.key.id) {
      axios.get('https://expense-api-5ehm.onrender.com/income', {
        params: { id: userData.key.id },
      })
        .then((response) => {
  ...
  const fetchExpense = useCallback(() => {
    if (userData && userData.key && userData.key.id) {
      axios.get('https://expense-api-5ehm.onrender.com/spending', {
        params: { id: userData.key.id },
      }).then((response) => {
  ...
  const fetchTotalMonthlyBudget = useCallback(() => {
    if (userData && userData.key && userData.key.id) {
      axios.get('https://expense-api-5ehm.onrender.com/monthly_budget', {
        params: {
          id: userData.key.id,
          month: current_Month,
          year: current_Year,
  ...
  const AddMonthlyBudget = useCallback(() => {
    if (userData && userData.key && userData.key.id) {
      axios.post('https://expense-api-5ehm.onrender.com/monthly_budget', {
        id: userData.key.id,
        income_id: incomeId,
        month: current_Month,
        year: current_Year,
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (cost && userData && userData.key && userData.key.id) {
      await axios.post('https://expense-api-5ehm.onrender.com/budgets', {
        id: userData.key.id,
  ...
  const handleDelete = async (id) => {
    if (userData && userData.key && userData.key.id) {
      await axios.delete('https://expense-api-5ehm.onrender.com/budget', {
        data: {
          budget_id: id,
  ...
  const handleEditFormSubmit = async (id) => {
    ...
    if(editCost&&userData && userData.key && userData.key.id){
      await axios.patch('https://expense-api-5ehm.onrender.com/budget', {
        user_id: userData.key.id,
