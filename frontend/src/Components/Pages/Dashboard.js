...
export default function Dashboard() {
  ...
  const fetchData = useCallback(async() => {
    if (userData && userData.key && userData.key.id) {
     await axios
        .get('https://expense-api-5ehm.onrender.com/dashboard', {
  ...
  const fetchIncome = useCallback(async() => {
    if (userData && userData.key && userData.key.id) {
      await axios.get('https://expense-api-5ehm.onrender.com/income', {
        params: { id: userData.key.id },
      })
        .then((response) => {
  ...
  const handleIncomeSubmit = async () => {
    try {
        if(!income){
          await axios.post('https://expense-api-5ehm.onrender.com/income', {
        ...
        await axios.patch('https://expense-api-5ehm.onrender.com/income', {
          userId: userData.key.id,
          income: newIncome,
        }).then((response) => {
          fetchIncome();
